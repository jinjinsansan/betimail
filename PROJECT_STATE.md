...
**最終更新**: 2026-09-18 (知識ベース反映 + 事実修正 + **包括診断と是正 — セクション 23 を必ず読む**。バックアップ自動化・監視・中断ジョブ回収を追加。外部 2 サイトのダウンは §22.3)
...
- **対象**: 仁氏が立ち上げた beti 投資家コミュニティのメンバー (1,066 名)。利権の事業主体は草野氏
- **目的**:
...
> 📌 本セクションの人物・経緯は 2026-09-09 に仁氏へ確認して訂正済み（セクション 21.2 参照）。正本は `ai_knowledge.md`。

beti は **仁氏** が 2024年に立ち上げた**個人投資家コミュニティ**。
**草野氏**が手掛ける事業の利権を、コミュニティ内で NFT として販売する形をとった。
4 種類の利権付き NFT を販売したが、2024年末からの日本国内オンラインカジノ規制、および
2026年8月の国内からのオンカジアクセス遮断で **ほぼ全事業が頓挫**。現在は買い取り (バイバック) 対応中。

### 販売した 4 種類の NFT (全て現状ほぼ実現困難)

| No | NFT | 販売時期 | 当初の利権 | 現状 |
|---|---|---|---|---|
| 1 | **会員権NFT** | 2024-01〜03 | 草野氏発案の WA7 (オンカジ) の利権 | ベガンスター社が取り決め・支払いを履行せず。**買い取り進行中**(微額) |
| 2 | **パチスロホイホイNFT** | 2024-04〜06 | デジタルパチスロのプロバイダー事業 | YOLO の RNG ライセンス取得停滞中。買い取り促進前段階 |
| 3 | **ラッキーマスタードNFT** | 2024-09〜2025-01 | WA7 の月次2000万円分配 | WA7 アクセス不可。代替源での配当も微々たる |
| 4 | **スペシャルマスタードNFT** | 2024-09〜2025-01 | クロフネカジノのスポーツブック | クロフネ自体が頓挫（2026年8月の国内アクセス遮断で確定）。買い取り促進準備中 |

### 重要な人物

- **仁氏 / 仁**: **beti コミュニティの立ち上げ**・国内運営、メルマガ送信者、本システムのユーザー
- **草野氏**: **beti が販売した利権の事業主体**。WA7 の発案者、デジタルパチスロを製造する Gp+1 のファウンダー
- **K氏 / K**: マレーシア在住。**クロフネカジノの開発主体**（beti 創設者ではない）
- **ベガンスター社**: WA7 の**開発を担いオーナー権を保有**する海外企業。草野氏側の意向・取り決め・支払いを履行しなかった
...
| **AI** | **DeepSeek** (`deepseek-chat` / openai 互換クライアント。`ai.py`・`config.py` 準拠) |
...
## 18. 2026-07-17 セッション引継ぎ（ポータル再構築 告知配信 + 実装計画）

### 18.1 このセッションでやったこと

1. **ポータル再構築の告知メルマガを全会員へ配信完了**
   - ジョブ **#12**「betiメルマガ通信」: **961/961 通送信・失敗 0**（エイリアス統合後の全会員）
   - 内容: ポータルサイト（betiダッシュボード）を自前再構築する / パチスロホイホイの買い取り・継続ボタンを設置予定 / 開発約3週間 / 新URLは完成後にメルマガで案内
2. **`ai_knowledge.md` セクション10 追加 + VPS 反映済み**
   - 告知の公知情報と想定問合せ8パターンの回答方針（完成日を約束しない / 買い取り金額は絶対に出さず needs_human / ベトナム法人は一般説明まで 等）
   - セクション2のホイホイ現状欄・セクション6用語集も更新。コンテナ再ビルド済み・/health OK
3. **`PORTAL_REBUILD_PLAN.md` 新規作成**（リポジトリ直下）— 次の大型案件の実装計画書
   - 正本データ: ルートの `ポータルサイト関連DB.zip` → `dashboard_20260715.sql`（git 追跡外、ダンプ日 2026-07-15）
   - 中身: 会員 3,266 行（実購入者はその一部）、会員権 547名/28,450口、ホイホイ 387名/22,990口、スペシャル 78名、残高保有 326名 計 $35,146.86、報酬明細 2,863 件、出金申請 29 件
   - 構成案: lucky 踏襲の ETL → `portal_*` テーブル → `/portal`（**白基調テーマ**）OTP ログイン → 買い取り/継続申請 + Telegram 通知 → 管理タブ PortalTab
   - **セクション8に仁氏への未確認仕様9点あり — 実装着手前に回答が必要**（買い取り金額の算定と表示有無 / 申請単位 / 「継続」の効果 / スペシャル78名の扱い（lucky側259名と重複）/ LEADER・DIGITAL_PACHISURO 各1名 / 分配cron要否 / ダンプの継続提供有無 等）

### 18.2 ⚠️ 発生した障害と復旧（再発防止必読）

**一括送信中にコンテナを再ビルドし、送信スレッドが 194/961 で中断した**（本セッションの Claude の操作ミス）。
- `ai_knowledge.md` 反映の `docker compose up -d --build betimail` を、ジョブ #12 送信中に実行してしまった
- 一括送信は betimail コンテナ内スレッドで動くため、再起動で死に、ジョブは `running` のまま停止する
- **復旧**: `bulk_job_targets`（961件スナップショット）と `sent_emails`（bulk_job_id=12, status='sent'）の差分 767 件のみ再送 → 二重送信なしで 961/961 完了
- 復旧スクリプト: VPS `/opt/betimail/data/resume_job12.py`（job_id を書き換えれば再利用可。`docker exec -d betimail python /app/data/resume_job12.py` でデタッチ実行、ログは同ディレクトリの .log）
- **教訓: デプロイ・再起動の前に必ず `bulk_send_jobs` に status='running' が無いか確認する**
- 恒久対応の候補（未実装）: 起動時に `running` 残留ジョブを検出して自動再開 or `error` に落として Telegram 通知

### 18.3 次セッションの最初にやること

1. **問合せ対応の監視**: 告知直後のためポータル関連の問合せが増える見込み。Telegram 承認カードと `pending_approvals` を確認。AI の回答品質に問題があれば `ai_knowledge.md` §10 を調整
2. **仁氏に `PORTAL_REBUILD_PLAN.md` セクション8 の9点を確認**（買い取り仕様が固まらないと Week 3 が組めない。閲覧ポータル先行公開のプランBあり）
3. 回答が揃い次第 **Week 1 に着手**: `tools/import_dashboard_dump.py`（ETL + 突合レポート）→ 仁氏の数値承認
4. 既存 DB との件数乖離に注意: 会員権 dump 547 vs betimail 568 / ホイホイ 387 vs 378 → 突合レポートで全件洗い出してから公開

### 18.4 状態スナップショット（2026-07-17 時点）

- git: `main` = ai_knowledge §10 + PORTAL_REBUILD_PLAN.md をコミット済み・push 済み
- VPS: 最新 `ai_knowledge.md` 反映済み、全コンテナ稼働、/health OK
- ローカル: pytest 72 件パス
- ポータル実装: **未着手（計画書のみ）**
...
### 20.5 セッション区切り時点の未決事項（2026-07-20 夕方・次セッションはここから）

1. **★スペシャルマスタード表示問題（仁氏の回答待ち）**:
   スペシャル保有はポータル(/portal)に表示されるが、それは旧ポータルDBに記録がある **78名/467口のみ**。
   betimail 名簿上のスペシャル保有者は **259名**（luckymustard 取引履歴由来）で、**約190名はポータルでスペシャルが見えない**（二重管理の乖離）。
   提示済みの選択肢: ①このまま公開し問合せは個別対応（AI は「枚数相違→needs_human」教育済み。推奨・様子見）②luckymustard 記録で補完表示（正本原則が崩れるため要整理）。
2. **仁氏の実機確認**: /portal（デザインはOK済み。ステーク/買い取り/出金＋Telegram通知の動作確認は未報告）・/white（「見ました」まで。操作確認は未報告）・管理タブ2つ
3. **告知メルマガ文面**: ポータル用の案は会話で提示済み（仁氏未承認）。白のダッシュボードの節を追記した最終版を公開直前に作成する
4. **afi 出金受付停止のタイミング**: 仁氏がベトナム法人/K氏と調整（「できます」との回答のみ。時期未定）
5. ポータル側の持ち越し: 会員リスト差分CSV判断（同一人物21組）/ LEADER・DIGITAL_PACHISURO 各1名の扱い / ダンプ継続提供の有無（§19.5・PORTAL_REBUILD_PLAN §8.2）

**現在の本番状態**: 3サイト稼働（/lucky=全会員公開、/portal・/white=goldbenchan 限定のソフトローンチ）。
全コンテナ正常・テスト102件パス・git は main = cbefdeb まで push 済み・作業ツリークリーン。
バックアップ: `betimail.db.bak-preportal`・`.env.bak-preportal`（VPS /opt/betimail/data/ と /opt/betimail/）。

---

## 21. 2026-09-09 第三者向け説明資料の改訂 + AI 知識ベースの事実修正（**新セッションは必ず読む**）

### 21.1 このセッションでやったこと

**システム（バックエンド・フロント・DB・VPS）には一切手を入れていない。** ドキュメント 2 点のみの更新。

1. **第三者向け説明資料「beti コミュニティ 経緯と現在のフォロー体制」を全面改訂**
   - Artifact URL: https://claude.ai/code/artifact/2b14a089-e0ba-4e7c-bdd1-19c34594ef4c
   - PDF 出力: `exports/beti_コミュニティ_経緯と現況_20260909.pdf`（旧版 `..._経緯と現況.pdf` は 2026-08-22 作成、同フォルダに残置）
   - 資料の HTML ソースはリポジトリ外（セッションのスクラッチパッド）。**次に改訂する時は Artifact を `action: "read"` で読んでから編集する**こと
2. **`ai_knowledge.md` の事実誤りを修正**（コミット `d1c9b63`）

### 21.2 ★判明した重大な事実誤り（旧ドキュメント・旧知識ベースは誤っていた）

仁氏に確認して確定した内容。**過去の PROJECT_STATE セクション 1・2 の記述も同じ誤りを含んでいる**（未修正）。

| 項目 | 誤（〜2026-08） | 正（2026-09-09 確定） |
|---|---|---|
| **beti の立ち上げ** | K氏が立ち上げた | **仁氏が立ち上げた**。草野氏が手掛ける事業の利権を、コミュニティ内で NFT として販売する形 |
| **WA7 の発案者** | K氏 | **草野氏** |
| **ベガンスター社** | WA7 の実権を握った | WA7 の**開発を担いオーナー権を保有**。草野氏側の意向・取り決め・支払いを履行しなかった |
| **事業主体** | K氏 | **草野氏**（WA7 発案・Gp+1 ファウンダー）。クロフネカジノの開発は K氏 |
| **ホイホイ販売期間** | 2024年4月〜7月 | **2024年4月〜6月** |
| **ラッキー販売期間** | 2024年後半 | **2024年9月〜2025年1月** |
| **スペシャル販売期間** | 2024年後半〜2025年1月 | **2024年9月〜2025年1月** |

### 21.3 新しい事実（2026-08 以降の情勢）

- **2026年8月、日本政府が CDN 事業者へ直接働きかけ、国内からオンラインカジノプラットホームへのアクセスを遮断。**
  2026年9月現在、Stake カジノ・Bitcasino 等の大手は国内から一切アクセス不可。
- **これによりクロフネカジノは完全に頓挫が確定**（スペシャルマスタードの対象事業）。
- 保証金の出所が判明: Sportsbet.io・Bitcasino への導入は、**Gp+1 が日本国内の企業から出資してもらった保証金**を積むことで実現していた（金額は資料に書かない方針）。

### 21.4 資料作成時の編集方針（次回改訂時も踏襲する）

仁氏の指示で確定した方針。

- **具体的な金額は原則すべて載せない**。今回削除したもの: 保証金2億円（3か所）／ダッシュボード残高合計 $35,146.86・$158,942.94／買い取り実績 10件・約2,569 USDT／WA7 月次2,000万円
- **出金の記載は増やさない**。資料の主旨は「① beti コミュニティの経緯 ②買い取りが進んでいること」。§04 の見出しは「買い取りの進め方」、出金は末尾1文のみ
- 「実権を握り」のような**断定的な表現は避け、履行の有無という事実**で書く（資料 §05 の原則と整合させる）
- §06「現在地」は**買い取りを先頭**に置く
- 冒頭メタ: 基準日 2026年9月9日 ／ 事業主体 草野氏 ／ コミュニティ運営 仁
- §02 に**4種の販売期間のガントチャート**（SVG）を追加済み
...
### 21.6 ついでに見つかった不整合（未修正）

- ~~**セクション 3.1 の「AI: Claude Opus 4.7」は誤り**~~ → **2026-09-18 修正済み**。実コード（`ai.py` / `config.py` / `requirements.txt`）は **DeepSeek（openai 互換クライアント、`deepseek-chat`）**。README も DeepSeek 表記
- ~~**`.venv/` が使用不能**（WSL 製）~~ → **2026-09-18 に作り直し済み**、pytest 102 件パス（§22.1）
- ~~Windows 側に Python が入っていない~~ → **誤り。Python 3.12.10 / 3.13.5 が利用可能**（ストアのスタブではなく実体あり）

### 21.7 PDF の作り直し方法

Artifact のページを Edge のヘッドレスで印刷している（資料には A4・余白16mm の印刷用 CSS が入っている）。

```powershell
# 1. Artifact の HTML を用意（action: "read" で取得 → doctype + <html data-theme="light"> でラップ）
# 2. 印刷
& "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" --headless --disable-gpu `
  --virtual-time-budget=20000 --no-pdf-header-footer `
  --print-to-pdf="V:\dev\Cusor\betimail\exports\beti_....pdf" "file:///<wrapped.html>"
```

`data-theme="light"` を付けないと、OS がダークテーマの時に**印刷結果が暗くなる**（`prefers-color-scheme` のブロックが印刷用トークンより優先されるため）。

### 21.8 状態スナップショット（2026-09-09）

- git: `main` = `d1c9b63`（`ai_knowledge.md` の修正）。**push 未実施**・作業ツリークリーン
- VPS: 未接続（このセッションでは一度も触っていない）。最終確認は 2026-07-20
- 本番: 3サイト稼働（`/lucky` 全会員公開、`/portal`・`/white` は管理者限定のソフトローンチ継続）
- テスト: 未実行（ローカルの venv が壊れているため）
...
1. **ローカル開発環境を復旧**（§21.6 の「venv が使えない」を解消）
   ...
   - `pip install -r requirements.txt` → **pytest 102 件パス**（引継ぎ書記載の件数と一致）
   - ⚠️ `requirements.txt` の `openai>=1.0.0` は上限なしのため、今回のローカルには **openai 3.15.0** が入った。
     本番コンテナは 2026-07 のビルド時点の 1.x 系である可能性が高い。テストは全て通ったが、
     AI 呼び出し周りで挙動差が出た場合はここを疑う（恒久対応の候補: バージョン固定）
2. **PROJECT_STATE の事実誤りを修正**（§21.5-3 の宿題）
   - セクション 1・2 の人物・経緯を §21.2 の確定内容へ差し替え（立ち上げ＝仁氏 / 事業主体＝草野氏 /
     K氏＝クロフネカジノの開発主体 / ベガンスター社＝オーナー権保有・履行せず / 4 種の販売期間）
...
## 23. 2026-09-18 包括診断と是正（**新セッションは必ず読む**）

システム全体を診断し、見つかった問題のうち手を動かせるものを直した。
**総合 63/100**（本番は動いているが、データ保全・認証情報・障害検知に穴があった）。

### 23.1 ★診断で判明した問題と対応状況

| # | 問題 | 状態 |
|---|---|---|
| 1 | **DB バックアップが自動化されていなかった**。最新は 2026-07-20 の移行前コピーで、直近 2 か月分はどこにも無かった | ✅ **是正済み**（§23.2） |
| 2 | **`ADMIN_PASSWORD` が露出済みの値のまま**（2026-05 から 4 か月） | ⛔ **未対応・仁氏の作業が必要**（§23.5） |
| 3 | **承認待ちの返信が 5 件、最古 125 日放置**。4 件は AI 生成失敗で下書きが空だった | 🟡 **下書きは復旧済み。送信は仁氏の確認待ち**（§23.3） |
| 4 | **受信メールが 2026-05-28 から途絶**。webhook 自体は正常（200 が 9,217 件）だが記録が無い | 🟡 **要検証**（§23.6） |
| 5 | **障害検知の仕組みが無い**（cron が 850 回失敗しても 5 日気づかず） | ✅ **是正済み**（§23.2） |
| 6 | **中断した一括送信ジョブが running のまま残る**（§18.2 が未対策だった） | ✅ **是正済み**（§23.2） |
| 7 | **Next.js に critical 脆弱性**（SSRF・キャッシュ汚染・DoS） | ✅ **是正済み**（15.5.25・監査クリーン） |
| 8 | **OS 更新 68 件未適用・再起動待ち**（127 日連続稼働） | ⛔ **未対応・再起動の可否判断が必要** |

### 23.2 実装した安全網（すべて本番反映済み）

**cron は 3 本体制になった**（`crontab.bak-20260918-prebackup` にバックアップ）:

```cron
0 20 * * *  lucky_distribute.py --notify-telegram          # 従来どおり
0 3  * * *  backup_db.py --verify --keep 14             --out-dir /app/data/backups --notify-telegram  # 新規
10 * * * *  health_watchdog.py --notify-telegram           # 新規
```

1. **日次 DB バックアップ**（`tools/backup_db.py`）
   - 出力先は **`/app/data/backups`**（マウント済みボリューム）。`/opt/betimail/backups` だと
     コンテナ内に書かれて再ビルドで消えるので注意
   - 初回実行を検証済み: 12.6MB gzip・`integrity_check` ok・**展開して全テーブル件数が本番と一致**
   - 失敗時のみ Telegram 通知（`--notify-success` で成功も通知可）
   - スクリプト側のバグ 2 件も修正: `with sqlite3.connect()` は commit するだけで close しないため
     中間ファイルを削除できずハンドルが残っていた／72MB を丸ごとメモリに載せていた（ストリーミング化）
   - ⚠️ **バックアップは同一ディスク上にある**。VPS 自体の障害には耐えられないので、
     将来は外部（S3 等）への転送を検討
2. **監視**（`tools/health_watchdog.py`・毎時）
   - バックアップ鮮度 / 分配の滞り / running のまま固まったジョブ / 放置された承認 / `/health` / ディスク
   - 異常時のみ通知、同一項目は 24 時間抑止（`/app/data/watchdog_state.json`）
   - 初回実行で **承認待ち 5 件・最古 125 日を即検知**した
3. **中断ジョブの回収**（`db.mark_interrupted_bulk_jobs` + `main.py` lifespan）
   - 起動時に `running` を `interrupted` へ落として Telegram 通知（プロセスが上がり直した時点で
     送信スレッドは必ず死んでいるため、起動時の running は常に中断を意味する）
   - 復旧は **`retry_failed_bulk.py <job_id> --resume-missing --confirm`**（新モード）。
     `bulk_job_targets` と `sent_emails` の差分だけを送るので二重送信にならない

テストは **126 件パス**（backup 5・watchdog 15・bulk recovery 4 を追加）。

### 23.3 承認待ち 5 件（仁氏の確認待ち）

2026-05-16〜05-28 に届いた問合せ。**4 件は AI 生成が失敗して下書きが空**だったため、
承認カードに中身が無く処理できない状態で放置されていた（これが 4 か月沈黙した直接の原因）。

`tools/regenerate_pending_drafts.py` で**下書きを作り直した**（送信はしていない）。
現在は 5 件すべてに 428〜581 文字の下書きがあり、管理画面の承認タブ／Telegram で確認できる。

| id | 受信日 | 件名 | 備考 |
|---|---|---|---|
| 5 | 05-16 | Re: 1/31出金申請済の件に関しまして | 出金の問合せ。**最優先** |
| 8 | 05-23 | Re: betiメルマガ通信 | |
| 10 | 05-24 | ベティのアカウントを他の方に移行したい | 名義変更。仁氏の判断が必要 |
| 11 | 05-25 | Re: betiメルマガ通信 | |
| 12 | 05-28 | こんにちは | 仁氏自身のテストと思われる |

⚠️ 生成された下書きは「ポータルは再構築中・完成次第メルマガで案内」と書く。
**ポータル公開後は `ai_knowledge.md` §10 を更新しないと古い案内を送り続ける**（§19.5-5）。

### 23.4 診断で確認した正常な点

- DB `integrity_check` **ok**。purchases 2,160 / lucky_rewards 255,466 / portal_members 838 /
  afi_members 904 / withdraw_requests 1,229 / sent_emails 6,783
- ラッキー分配は毎晩正常（2026-09-17 も 393 名 / 685 枚 / 350.86 USDT）
- AI は正常（DeepSeek へライブ呼び出しで確認。**本番の `openai` は 3.15.0**。
  `requirements.txt` が `openai>=1.0.0` と無固定のため再ビルドで上がった。固定を推奨）
- Caddy のセキュリティヘッダ適切・TLS は 2026-12-09 まで有効で自動更新
- `.env` は 600 権限・`.gitignore` は会員 PII を正しく除外
- リソース余裕あり（メモリ 616/1956MB・ディスク 48%）
- 会員は実際に使っている（9/17・9/18 に OTP ログインのメール送信イベントを確認）

### 23.5 ★仁氏にお願いする作業（未対応）

**1. 管理者パスワードのローテーション（最優先）**

`ADMIN_PASSWORD` がチャット履歴に出た値のままで、管理画面は全会員の個人情報と送信機能に到達できる。
Claude 側は `.env` への書き込みが権限で拒否されるため実行できない。
**Claude Code 経由ではなくご自身のターミナル**で実行すること（`!` 実行だと新しい値が履歴に残る）:

```bash
ssh betimail-vps
cd /opt/betimail
cp .env .env.bak-prerotate
NEWPW=$(openssl rand -base64 32 | tr -dc 'A-Za-z0-9' | cut -c1-24)
sed -i "s|^ADMIN_PASSWORD=.*|ADMIN_PASSWORD=$NEWPW|" .env
echo "新しいパスワード: $NEWPW"   # パスワード管理ツールに保存してから画面を閉じる
docker compose up -d betimail
```

⚠️ **トークンの署名鍵は `ADMIN_PASSWORD` から導出している**（`auth.py:_derive_secret`）。
変更すると**会員の /lucky ログインセッションも全て失効**する（OTP で再ログインすれば復帰）。

- 設計上の弱点: 会員セッションの安全性が管理者パスワードに結びついている。
  本来は署名鍵を分けるべき（将来の改善候補）
- 併せてローテーション推奨: Resend API キー / Webhook シークレット / Telegram ボットトークン / DeepSeek キー
- `LUCKY_ADMIN_*` と `NFTPORTAL_ADMIN_*` は**対象サイトが消滅したので削除してよい**
  （`daily_lucky_reward.py` と `explore_*.py` でしか使われておらず、どちらも死んだサイト向け）

**2. OS 更新と再起動**

68 件未適用・再起動待ち（127 日連続稼働）。再起動中は API が数分止まる。
実行するなら `apt update && apt upgrade -y && reboot`。**一括送信の予定が無い時間帯**に。

### 23.6 受信メール途絶の検証方法（未解決）

`received_emails` の最新は **2026-05-28**。webhook は正常に動いており（`/webhook/email` へ 200 が
9,217 件、直近は 2026-09-18）、その大半は**送信側の配信イベント**。
961 名へメルマガを 2 回送って返信が 1 通も無いのは考えにくく、
**Resend の inbound routing が壊れている可能性**がある。

検証: **仁氏の Gmail から `support@betimail.uk` にテストメールを送る** → 数分後に
管理画面の受信履歴に出れば正常。出なければ Resend ダッシュボードの Inbound Routing 設定を確認する。

---

**運営: 仁氏 (`goldbenchan@gmail.com`)**
**開発支援: Claude Opus 4.7 (Anthropic) / v1.2.0 は Claude Opus 4.8 / ポータル再構築計画は Claude Fable 5 / 2026-09 以降の保守は Claude Opus 5**
