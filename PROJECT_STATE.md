...
**最終更新**: 2026-09-09 (第三者向け説明資料の改訂 + AI 知識ベースの事実誤りを修正。セクション 21 参照。システム自体は 2026-07-20 から変更なし)
...
3. **PROJECT_STATE セクション 1・2 の人物・経緯の記述が §21.2 の誤りを含んだまま**。読む人が混乱するので、次セッションで修正する
4. **`git push` 未実施**（`d1c9b63` はローカルのみ）

### 21.6 ついでに見つかった不整合（未修正）

- **セクション 3.1 の「AI: Claude Opus 4.7 (Anthropic API + prompt caching)」は誤り**。実コード（`ai.py` / `config.py` / `requirements.txt`）は **DeepSeek（openai 互換クライアント、`deepseek-chat`）**。README も DeepSeek 表記。ドキュメント側が古い
- **`.venv/` が使用不能**。WSL 上の `/mnt/e/dev/Cusor/betimail` で作られたもので、現在の `V:\dev\Cusor\betimail` からは実行できない（`pyvenv.cfg` の home=/usr/bin）。pytest を回すには venv 作り直しが必要
- Windows 側に Python が入っていない（`WindowsApps\python.exe` はストアのスタブ）
